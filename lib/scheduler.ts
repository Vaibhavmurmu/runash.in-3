...
import { readSchedulesFromDisk, writeSchedulesToDisk, getDashboardData } from "./data-source"
...
const SCHEDULES =
  (global as any).__schedules || new Map<string, { schedule: Schedule; task: cron.ScheduledTask | null }>()
...
async function runReport(s: Schedule) {
  ...
  const rows = [
    ["date", "viewers", "followers", "revenue"],
    ...data.overview.map((r) => [r.date, String(r.viewers), String(r.followers), String(r.revenue)]),
  ]
...
export async function addSchedule({
  ...
  recipients,
  format,
}: { name: string; frequency: Schedule["frequency"] | string; recipients?: string; format?: string }) {
  const id = uuidv4()
  const s: Schedule = {
    id,
    name,
